...
1. **開始**:播放「資料夾 + 這一列的音效檔名稱」,音量套用這一列的 `MaxS`。先播「開場片段」(`SStart`~`SEnd`,只播一次),播到結束時間就自動接著播「循環片段」(`CStart`~`CEnd`,反覆循環,播到結束時間就跳回開始時間重播),直到按下暫停或停止。
   ...
   - 如果循環片段的開始時間跟結束時間相同(沒有長度、不用播放),開場片段還是會正常播完,但播完後會直接觸發停止,不會進入循環片段——因為循環片段沒有長度的話,每次輪詢都會把播放位置定位回同一個點,等於不斷在同一個時間點重複定位,會一直產生破音(見 `PlaybackController.Timer_Tick`/`Player_MediaEnded`)。
...
「別的列」的鎖定(`AudioMapRow.IsPlaybackLocked`)由 `MainWindow` 在開始播放時對其餘每一列設成 `true`(`LockOtherRows`)。解除鎖定(`UnlockAllRows`/`SetGlobalControlsEnabled(true)`/補一次 `Preload`)統一放在 `Playback_PositionChanged` 偵測到 `_playback.PlayingRow` 變成 `null` 的地方處理,不是只在使用者按「停止」的 `MaxVolumeControl_StopRequested` 裡面做——因為 `PlaybackController` 自己也會觸發停止(循環片段沒有長度、或播放失敗時,見上面「播放」章節),這些情況一樣要解除鎖定,不然畫面會卡在反灰狀態。同一時間只會有一列在播放/暫停,因為其他列在那期間全部反灰、點不到「開始」。
